  ...
  <footer class="bg-gray-900 text-gray-400 pt-12 pb-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div>
          <div class="flex items-center gap-3 mb-4">
            <img
              ...
              class="h-[3.75rem] w-auto object-contain brightness-0 invert"
            />
            <div>
              <p class="text-white font-bold leading-tight">{{ DOCTOR.shortName }}</p>
              <p class="text-gray-500 text-xs">{{ DOCTOR.crm }}</p>
            </div>
          </div>
          <p class="text-sm leading-relaxed">
            Atendimento médico humanizado em Belém, Pará. Comprometido com a saúde e
          ...
          <div class="flex items-center gap-3 mt-5">
            ...
            <!-- Ícones aparecem após preencher DOCTOR.instagram e DOCTOR.facebook em config.js -->
            <span v-if="!DOCTOR.instagram && !DOCTOR.facebook" class="text-xs text-gray-600 italic">
              Preencha as redes sociais em config.js
            </span>
          </div>
        </div>
          ...
          <ul class="space-y-2">
            <li v-for="item in navLinks" :key="item.label">
              <a
                ...
                class="text-sm hover:text-white transition-colors cursor-pointer"
              >
                {{ item.label }}
              </a>
            </li>
        ...
        <div>
          ...
          <ul class="space-y-3">
            <li class="flex items-center gap-2 text-sm">
              ...
              <a :href="whatsappUrl()" target="_blank" rel="noopener noreferrer" class="hover:text-white transition-colors">
                {{ DOCTOR.phone }}
              </a>
            </li>
            <li class="flex items-center gap-2 text-sm">
      ...
      <div class="border-t border-gray-800 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 text-xs text-gray-600">
        ...
        <p>{{ DOCTOR.crm }} · {{ DOCTOR.specialty }} · Belém, PA</p>
      </div>
    </div>
  </footer>
</template>
